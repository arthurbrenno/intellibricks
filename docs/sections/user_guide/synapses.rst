...
.. code-block:: python
   ...
   gemini_synapse = Synapse.of(AIModel.GEMINI_PRO) # Or "google/genai/gemini-pro-experimental"
...
.. code-block:: python
   ...
   openai_synapse = Synapse.of(AIModel.GPT_4o) # Or "openai/api/gpt-4o"
...
.. code-block:: python
   ...
   groq_synapse = Synapse.of(AIModel.MIXTRAL_8x7B_32768) # Or "groq/api/mixtral-8x7b-32768"

   # Ensure you have set your GROQ_API_KEY environment variable.

**Available Models (AIModel Enum)**

IntelliBricks provides an ``AIModel`` enum in ``intellibricks.llms.constants`` that lists supported models with easy-to-use identifiers. You can use these enum values with ``Synapse.of()``:

.. code-block:: python

   from intellibricks.llms.constants import AIModel

   print(AIModel.GEMINI_PRO) # Output: google/genai/gemini-pro-experimental
   print(AIModel.GPT_4o)     # Output: openai/api/gpt-4o
   print(AIModel.MIXTRAL_8x7B_32768) # Output: groq/api/mixtral-8x7b-32768

Refer to the ``AIModel`` enum in the source code for a complete list of supported models and their identifiers.
...
.. code-block:: python
   ...
   from intellibricks.llms.types import UserMessage, AssistantMessage
...
.. code-block:: python
   ...
   from intellibricks import SynapseCascade

   synapse_cascade = SynapseCascade(
       synapses=[
           Synapse.of(AIModel.GEMINI_PRO),       # Primary Synapse (e.g., Google Gemini Pro)
           Synapse.of(AIModel.GPT_3_5_TURBO),   # Fallback Synapse 1 (e.g., OpenAI GPT-3.5 Turbo)
           Synapse.of(AIModel.MIXTRAL_8x7B_32768), # Fallback Synapse 2 (e.g., Groq Mixtral)
       ]
   )
...
.. code-block:: python
   ...
   from intellibricks import TextTranscriptionSynapse
   from intellibricks.llms.constants import TranscriptionModelType

   whisper_synapse = TextTranscriptionSynapse.of(TranscriptionModelType.WHISPER_1) # Or "openai/api/whisper-1"
...
.. code-block:: python
   ...
   transcription_cascade = TextTranscriptionsSynapseCascade(
       synapses=[
           TextTranscriptionSynapse.of(TranscriptionModelType.WHISPER_1), # Primary transcription service
